...
export class MessageLabelsService {
  ...
  async run(messageId: number, systemLabel: SystemLabel, isRemove: boolean): Promise<Message> {
    ...
    throwUnless(
      hasLabel === isRemove,
      new BadRequestException(`Label ${systemLabel} ${isRemove ? 'does not exist' : 'already exists'}`),
    );

    const gmailLabelId = (await this.settingService.get<UpdateDto>(SettingKey.EmailLabels))[systemLabel];
    throwUnless(gmailLabelId, new BadRequestException(`Gmail label mapping not found for ${systemLabel}`));

    const gmail = await this.googleapisService.getGmailClient();
    await gmail.users.messages.modify({
      userId: 'me',
      id: message.gmailMessageId,
      requestBody: { addLabelIds: isRemove ? [] : [gmailLabelId], removeLabelIds: isRemove ? [gmailLabelId] : [] },
    });
    ...
    await this.messageRepository.update(message.id, { systemLabels: newSystemLabels });
